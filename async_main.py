...
async def process_page(session, block_transactions_url, page):
    response = await fetch(session, f'{block_transactions_url}&p={page}')
...
async def get_data(url: str, start_block: str, end_block: str) -> list:
    ...
    async with aiohttp.ClientSession() as session:
        for block in blocks:
            ...
            response = await fetch(session, block_transactions_url)
            response_soup = BeautifulSoup(response, 'lxml')
            pages_count = response_soup.find_all('li', class_='page-item disabled')
            filter_page_counts = list(filter(lambda x: x.find('span', class_='page-link text-nowrap'), pages_count))
            if filter_page_counts:
                ...
                for page in range(1, int(pages) + 1):
                    ...
                    tasks.append(process_page(session, block_transactions_url, page))
...
loop.run_until_complete(get_data(config_data['url'], config_data['start_block'], config_data['end_block']))
loop.close()